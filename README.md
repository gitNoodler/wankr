...
2. **Cloudflare Tunnel** (Zero Trust → Networks → Tunnels): deploy `tunnel/Dockerfile` as a separate Railway service (`wankr-tunnel`) with `CLOUDFLARE_TUNNEL_TOKEN` set as a Railway variable. Public hostname: **wankrbot.com** → **http://wankr-backend.railway.internal:5000** (Railway internal networking). Do **not** use `127.0.0.1` — the tunnel runs in a separate container.
